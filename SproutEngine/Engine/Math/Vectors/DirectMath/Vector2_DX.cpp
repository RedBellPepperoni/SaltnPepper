...
namespace SproutEngine::Maths
{
	...
	inline SPROUT_VECTOR2_DX& SPROUT_VECTOR2_DX::operator+=(const SPROUT_VECTOR2_DX& _vector) noexcept
	{
		...
		XMStoreFloat2(&this->xmvector, finalVector);
		return *this;
	}
	...
	{
		using namespace DirectX;
		const XMVECTOR minVector = XMLoadFloat2(&_firstVector.xmvector);
		const XMVECTOR maxVector = XMLoadFloat2(&_secondVector.xmvector);
	...
	inline void SPROUT_VECTOR2_DX::Normalize(SPROUT_VECTOR2_DX& _result) const noexcept
	{
		...
		const XMVECTOR normalizedVector = XMVector2Normalize(vectorThis);
		XMStoreFloat2(&_result.xmvector, normalizedVector);
	}
	...
	{
		using namespace DirectX;
		const XMVECTOR vectorOne = XMLoadFloat2(&_vectorOne.xmvector);
		const XMVECTOR vectorTwo = XMLoadFloat2(&_vectorTwo.xmvector);
		const XMVECTOR lerpedVector = XMVectorLerp(vectorOne, vectorTwo, _lerpFactor);
		XMStoreFloat2(&_result.xmvector, lerpedVector);
	}
	...
	{
		using namespace DirectX;
		const XMVECTOR vector = XMLoadFloat2(&_vector.xmvector);
		const XMVECTOR vectorMin = XMLoadFloat2(&_vectorMin.xmvector);
		const XMVECTOR vectorMax = XMLoadFloat2(&_vectorMax.xmvector);
		const XMVECTOR clampedVector = XMVectorClamp(vector, vectorMin, vectorMax);
		XMStoreFloat2(&_result.xmvector, clampedVector);
	}
	...
	inline void SPROUT_VECTOR2_DX::Min(const SPROUT_VECTOR2_DX& _vectorOne, const SPROUT_VECTOR2_DX& _vectorTwo, SPROUT_VECTOR2_DX& _result) noexcept
	{
		...
		XMStoreFloat2(&_result.xmvector, minVector);
	}
	...
	inline void SPROUT_VECTOR2_DX::Max(const SPROUT_VECTOR2_DX& _vectorOne, const SPROUT_VECTOR2_DX& _vectorTwo, SPROUT_VECTOR2_DX& _result) noexcept
	{
		...
		XMStoreFloat2(&_result.xmvector, finalVector);
	}
	...
	{
		using namespace DirectX;
		const XMVECTOR incidentVector = XMLoadFloat2(&_incidentVector.xmvector);
		const XMVECTOR normalVector = XMLoadFloat2(&_normalVector.xmvector);
		const XMVECTOR reflectedVector = XMVector2Reflect(incidentVector, normalVector);
		XMStoreFloat2(&_result.xmvector, reflectedVector);
	}
	...
	void SPROUT_VECTOR2_DX::Refract(const SPROUT_VECTOR2_DX& _incidentVector, const SPROUT_VECTOR2_DX& _normalVector, float _refractionIndex, SPROUT_VECTOR2_DX& _result) noexcept
	{
		...
		XMStoreFloat2(&_result.xmvector, refractedVector);
	}
	...
	{
		SPROUT_VECTOR2_DX result;
		Refract(_incidentVector, _normalVector, _refractionIndex, result);
		return result;
	}
	...
	void SPROUT_VECTOR2_DX::Transform(const SPROUT_VECTOR2_DX& _vector, const Quaternion& _quaternion, SPROUT_VECTOR2_DX& _result) noexcept
	{
		...
		XMStoreFloat2(&_result.xmvector, transformedVector);
	}
	...
	{
		using namespace DirectX;
		const XMVECTOR vectorOne = XMLoadFloat2(&_firstVector.xmvector);
		const XMVECTOR vectorTwo = XMLoadFloat2(&_secondVector.xmvector);
		const XMVECTOR addedVector = XMVectorAdd(vectorOne, vectorTwo);
		SPROUT_VECTOR2_DX finalVector;
		XMStoreFloat2(&finalVector.xmvector, addedVector);
		return finalVector;
	}
	...
	SPROUT_VECTOR2_DX operator-(const SPROUT_VECTOR2_DX& _firstVector, const SPROUT_VECTOR2_DX& _secondVector) noexcept
	{
		...
		SPROUT_VECTOR2_DX finalVector;
		XMStoreFloat2(&finalVector.xmvector, subtractedVector);
		return finalVector;
	}
	...
	SPROUT_VECTOR2_DX operator*(const SPROUT_VECTOR2_DX& _firstVector, const SPROUT_VECTOR2_DX& _secondVector) noexcept
	{
		...
		SPROUT_VECTOR2_DX finalVector;
		XMStoreFloat2(&finalVector.xmvector, multipliedVector);
		return finalVector;
	}
	...
	SPROUT_VECTOR2_DX operator*(float _float, const SPROUT_VECTOR2_DX& _vector) noexcept
	{
		 return _vector * _float;
	}
	...
	SPROUT_VECTOR2_DX operator/(const SPROUT_VECTOR2_DX& _firstVector, const SPROUT_VECTOR2_DX& _secondVector) noexcept
	{
		...
		SPROUT_VECTOR2_DX finalVector;
		XMStoreFloat2(&finalVector.xmvector, dividedVector);
		return finalVector;
	}
